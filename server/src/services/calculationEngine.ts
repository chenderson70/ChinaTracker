...
function getPlayerExecutionExerciseDutyDays(exercise: { startDate?: string | null; endDate?: string | null; defaultDutyDays?: number | null }): number | null {
...
export function calculateBudget(exercise: any, rates: RateInputs): BudgetResult {
  ...
  for (const ub of exercise.unitBudgets || []) {
    ...
    for (const pg of ub.personnelGroups || []) {
      ...
      const hasGroupRental = (pg.rentalCarCount || 0) > 0 || (pg.rentalCarDays || 0) > 0;
